    ...
    public function index(Request $request): Response
    {
        ...
        if($request->get('email')) {
            ...
            if($user) {
                ...
                $reset_password->setCreatedAt(new \DateTimeImmutable('Europe/Paris'));
                ...
                $content = "Bonjour ".$user->getFirstName()."<br/>Vous avez demandé à rénitialiser votre mot de passe sur le site Delta Ecommerce.<br/><br/>";
                $content .= "Merci de bien vouloir cliquer sur le lien suivant pour <a href='".$url."'>mettre à jour votre mot de passe</a>.";

                $mail = new Mailjet();
                $mail->send($user->getEmail(), $user->getFirstName().' '.$user->getLastName(), 'Rénitialiser votre mot de passe sur la boutique Delta Ecommerce', $content );

                $this->addFlash('success', 'Vous allez recevoir dans quelques secondes un mail avec la procédure pour rénitialiser votre mot de passe.');
            }else {
